...
--------
  ...
  3. For each quadricule that spatially intersects the ortho:
       ...
       e. Write  dist/masks/langsam/<TILE_ID>_core.geojson
...
-----
  ...
  python run_langsam.py --ortho sandbox/data/ortho/ortofoto.tif --grid res/data/shp/quadricules-utm-v1r0-2021/quadricules-utm-v1r0-2021.shp

  # Process a single tile (useful for debugging):
  python run_langsam.py --ortho sandbox/data/ortho/ortofoto.tif --tile-id 31TCF4158

  # Force reprocessing even if output already exists:
  python run_langsam.py --ortho sandbox/data/ortho/ortofoto.tif --overwrite
...
# Load module config (overrides global if present)
...
DEFAULT_GRID = PROJECT_ROOT / "res" / "data" / "shp" / "test_quadricules.geojson"
DEFAULT_ORTHO = PROJECT_ROOT / config.get("ortho_path", "sandbox/data/ortho")

MIN_POLYGON_AREA = float(config.get("min_polygon_area_m2", 1.0))
MAX_POLYGON_AREA = float(config.get("max_polygon_area_m2", 50000.0))
MERGE_DISTANCE = float(config.get("merge_distance_m", 5.0))
...
def filter_polygons(polygons: list, min_area: float, max_area: float) -> list:
    """Remove polygons outside the area bounds (noise / false positives)."""
...
def run_langsam(
    ...
    """
    Run LangSAM with every PV prompt. Returns a flat list of (polygon, score) tuples
    ...
    for prompt in PV_PROMPTS:
        ...
        for result in results:
            ...
            if scores is not None and len(scores) == len(polys):
                all_results.extend(zip(polys, scores.tolist()))
            else:
                all_results.extend(zip(polys, [1.0] * len(polys)))
...
def to_geojson(results: list, tile_id: str, region: str) -> dict:
    """Merge overlapping polygons and build a GeoJSON FeatureCollection with scores."""
    if not results:
        return {
        ...
        "features": [
            {
                ...
                "properties": {
                    ...
                    "crs": "EPSG:25831",
                    "area_m2": g.area,
                    "avg_score": round(sum(avg_scores) / len(avg_scores), 3),
                },
            }
...
def process_tile(
    ...
    try:
        ...
        results = run_langsam(pil_img, transform, box_threshold, text_threshold)

        polygons = [r[0] for r in results]
        polygons = filter_polygons(polygons, MIN_POLYGON_AREA, MAX_POLYGON_AREA)
        polygons = merge_nearby_polygons(polygons, MERGE_DISTANCE)
        results_filtered = [(p, 1.0) for p in polygons]

        summary["core"] = len(results_filtered)
        save_geojson(
            to_geojson(results_filtered, tile_id, "core"),
    ...
    if process_donut:
        ...
        if tmp_d is not None:
            try:
                ...
                results_d = run_langsam(pil_d, transform_d, box_threshold, text_threshold)

                polygons_d = [r[0] for r in results_d]
                polygons_d = filter_polygons(polygons_d, MIN_POLYGON_AREA, MAX_POLYGON_AREA)
                polygons_d = merge_nearby_polygons(polygons_d, MERGE_DISTANCE)
                results_d_filtered = [(p, 1.0) for p in polygons_d]
...
@click.option(
    ...
    help="GeoJSON or GPKG/SHP with UTM 1km quadricule polygons. "
         "Defaults to res/data/shp/test_quadricules.geojson (2 test tiles). "
         "For a full run, supply the complete ICGC UTM 1km grid, e.g. "
         "res/data/shp/quadricules-utm-v1r0-2021/quadricules-utm-v1r0-2021.shp",
)
@click.option(
...
def main(ortho_path, grid_path, tile_id, process_donut, overlap_fraction,
    ...
    georeferenced GeoJSON masks to dist/masks/langsam/.
